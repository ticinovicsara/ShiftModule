...
describe('session-type module (e2e)', () => {
  ...
  it('GET /session-types should support happy + 401 + 403', async () => {
  ...
  it('GET /session-types/course/:courseId should support happy + 401 + 403', async () => {
    const ok = await request(ctx.app.getHttpServer())
      .get(`/session-types/course/${seedIds.courses.osnove1}`)
      .set(authHeader(UserRole.ADMIN));
    expect(ok.status).toBe(200);
    expectEnvelope(ok.body);

    const noToken = await request(ctx.app.getHttpServer()).get(
      `/session-types/course/${seedIds.courses.osnove1}`,
    );
    expect(noToken.status).toBe(401);
  ...
  it('POST /session-types should support happy + 401 + 403 + 400', async () => {
    const ok = await request(ctx.app.getHttpServer())
      .post('/session-types')
      .set(authHeader(UserRole.ADMIN))
  ...
  it('PATCH /session-types/:id should support happy + 401 + 403 + 400 + 404', async () => {
    const ok = await request(ctx.app.getHttpServer())
      .patch(`/session-types/${seedIds.sessionTypes.lab}`)
      .set(authHeader(UserRole.ADMIN))
  ...
  it('DELETE /session-types/:id should support happy + 401 + 403 + 404', async () => {
    ...
    const ok = await request(ctx.app.getHttpServer())
      .delete(`/session-types/${created.body.data.id}`)
      .set(authHeader(UserRole.ADMIN));
    expect(ok.status).toBe(200);
    expectEnvelope(ok.body);

    const notFound = await request(ctx.app.getHttpServer())
      .delete(`/session-types/${seedIds.notFound.sessionType}`)
      .set(authHeader(UserRole.ADMIN));
    expect(notFound.status).toBe(404);
    ...
    const wrongRole = await request(ctx.app.getHttpServer())
      .delete(`/session-types/${seedIds.sessionTypes.lab}`)
      .set(authHeader(UserRole.PROFESSOR));
    expect(wrongRole.status).toBe(403);
  });
});
