...
describe('swap-request module (e2e)', () => {
  ...
  it('GET /swap-requests/student/requests should support happy + 401 + current role behavior', async () => {
  ...
  it('POST /swap-requests/student/requests should support 401 + 400 current behavior', async () => {
  ...
  it('POST /swap-requests/student/requests/:id/confirm-partner should support current lifecycle + 401 + 403 + 404', async () => {
    const ok = await request(ctx.app.getHttpServer())
      .post(
  ...
  it('POST /swap-requests/student/requests/:id/satisfaction should support student feedback on resolved request', async () => {
    const ok = await request(ctx.app.getHttpServer())
      .post(
        `/swap-requests/student/requests/${seedIds.swapRequests.request1}/satisfaction`,
      )
      .set(authHeader(UserRole.STUDENT))
      .send({ accepted: false });
    expect([200, 201]).toContain(ok.status);
    expectEnvelope(ok.body);
    expect(ok.body.data.satisfiedWish).toBe(false);
    ...
    const notFound = await request(ctx.app.getHttpServer())
      .post(
        `/swap-requests/student/requests/${seedIds.notFound.swapRequest}/satisfaction`,
      )
      .set(authHeader(UserRole.STUDENT))
      .send({ accepted: true });
    expect(notFound.status).toBe(404);
  ...
  it('GET /swap-requests/professor/requests should include satisfiedWish for resolved requests', async () => {
    const setSatisfaction = await request(ctx.app.getHttpServer())
      .post(
        `/swap-requests/student/requests/${seedIds.swapRequests.request1}/satisfaction`,
      )
      .set(authHeader(UserRole.STUDENT))
      .send({ accepted: false });
    expect([200, 201]).toContain(setSatisfaction.status);
    ...
    const requestItem = (
      response.body.data as Array<{
        id: string;
        satisfiedWish?: boolean | null;
      }>
    ).find((item) => item.id === seedIds.swapRequests.request1);

    expect(requestItem).toBeDefined();
    expect(requestItem).toHaveProperty('satisfiedWish');
    expect(requestItem?.satisfiedWish).toBe(false);
  });
